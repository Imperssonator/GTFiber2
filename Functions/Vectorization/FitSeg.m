function [xy_cur,curv,MSE,L] = FitSeg(ims,xy_init,fibSegImg,fiberStep,params)
...
for k = 1:params(7)
    ...
    vf(:,1) = vf(:,1) + k2*v_start; %*(int_start-bgInt)/(fiberIntensity-bgInt);
    vf(:,end) = vf(:,end) + k2*v_end; %*(int_end-bgInt)/(fiberIntensity-bgInt);
