function ims = op2d_new(ims)

% A modified version of FiberApp's OrderParameter2D that computes the
% aligned fraction as:
% (1 - area under the random decay curve / area under image decay curve)
...
% f=figure('NumberTitle', 'off', 'Name', ['OP2D ' datestr(now, 'HH:MM:SS dd/mm/yy')]);
% ax=gca;
% hold(ax,'on')
% area_rand = area(ax,xdata, S_rand);
% area_rand.FaceColor = [1 0 0];
% area_rand.FaceAlpha = 0.3;
% area_im = area(ax,xdata, S_im);
% area_im.FaceColor = [0 0 1];
% area_im.FaceAlpha = 0.3;
% plot(ax,xdata,S_im,'.',xdata,S_rand,'.')
...
% isFitData=1;
% if isFitData
%     fun = @(x,xdata) (1-x(1)).*S_rand + x(1).*((1-x(2)).*exp(-xdata./(2.*x(3)))+x(2));
%     OPTIONS = optimoptions('lsqcurvefit','Algorithm','levenberg-marquardt');
%     res = lsqcurvefit(fun, [0.6, 0.1, 100], xdata, S_im, [], [], OPTIONS);
%     exfl=1;
%     if exfl == 1
%         fit_y = (1-res(1)).*S_rand + res(1).*((1-res(2)).*exp(-xdata./(2.*res(3)))+res(2));
%         fit_align = res(1).*((1-res(2)).*exp(-xdata./(2.*res(3)))+res(2));
%         fit_rand = (1-res(1)).*S_rand;
%         plot(xdata, S_im, '.', ...
%             xdata, S_rand, '.', ...
%             xdata, S_im-S_rand, '.', ...
%             [0, xdata], [1, fit_y], '-', ...
...
%             xdata, fit_rand, '-' ...
%             );
%         legend('S_{im}', 'S_{rand}', 'S_{align}', 'Location', 'North');
%         text('Units', 'normalized', 'Position', [0.7 0.88], ...
